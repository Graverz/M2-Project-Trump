...
```{r fig.height=4, fig.width=10}
network <- as.character(seq(100,1000, by=100)) %>% as.factor() %>% reorder(sort(as.numeric(.)))

lll <- list(air_n100, air_n200, air_n300, air_n400,air_n500, air_n600, air_n700, air_n800,air_n900,air_n1000)
...
```{r fig.height=10, fig.width=10}
a <- air_n300 %N>% 
  filter(region %in% c("Europe", "North America", "Asia")) %>% 
  ggraph(layout = "stress") + 
  #geom_edge_link(alpha=0.5) + 
  geom_node_point(aes(color=region), size=5) +
  scale_color_brewer(palette="RdYlGn") +
  geom_node_text(aes(label = short), size=1) +
  labs(title="Airport clusters 300 km") + facet_nodes(~region)
...
b <- air_n500 %N>% 
  ...
  scale_color_brewer(palette="RdYlGn") +
  geom_node_text(aes(label = short), size=1) +
  labs(title="Airport clusters 500 km") + facet_nodes(~region)
...
c <- air_n1000 %N>% 
  ...
  scale_color_brewer(palette="RdYlGn") +
  geom_node_text(aes(label = short), size=1) +
  labs(title="Airport clusters 1000 km") + facet_nodes(~region)
...
air_n300 %>% 
  ...
  geom_edge_link(alpha=0.5) + 
  geom_node_point(aes(color=region),alpha=0.8, size=7) +
  scale_color_brewer(palette="RdYlGn") +
  geom_node_text(aes(label = short), size=0.7, color="white") +
  labs(title="Color by centrality - 100 km") + theme_void()
```
...
air_n500 %N>% 
  ...
  scale_color_continuous(guide = "legend") + 
  geom_node_text(aes(label = short), size=0.7, color="white") +
  labs(title="Color by centrality - 500 km") + theme_void()
```
